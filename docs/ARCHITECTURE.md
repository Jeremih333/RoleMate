...
Наборы premium-эмодзи импортируются ссылкой `t.me/addemoji/…`, отправленной боту
в личные сообщения: `getStickerSet` возвращает набор, в `custom_emoji_packs` и
`custom_emoji` попадают только идентификаторы файлов. Набор общий для всех и не
требует RoleMate Premium. Байты каждого эмодзи скачиваются один раз и хранятся в
`custom_emoji_assets` (TGS распаковывается в Lottie при сохранении): иначе
отрисовка пикера означала бы сотни обращений к Telegram, и он начинает
throttling. Плановая задача наполняет хранилище небольшими партиями, укладываясь
в лимит subrequest'ов бесплатного плана. В шапку профиля допускаются только
эмодзи с `needs_repainting` — они рисуются маской и принимают цвет шапки; это и
оформление шапки в целом остаются Premium и снимаются сразу по истечении
подписки.
