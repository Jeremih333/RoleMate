...
> Обновление 29 июля 2026: миграции `0008`–`0011`, публикации, рейтинги,
> промокоды, обязательные подписки, безопасная роль модератора, поиск по тегам,
> Markdown, Premium-медиа и исправления `/menu`/`RATE_LIMITED` опубликованы в
> production. Актуальные версии, 52 автоматических теста, 51 E2E и оставшиеся
> ручные gates зафиксированы в
> `FINAL_VERIFICATION_REPORT.md`; более ранние числа ниже сохранены как история
> аудита.
...
| Mini App работает                          | Реализовано, ждёт production-проверки | production URL и assets отвечают 200, 51 Playwright E2E зелёный; invalid-signature auth после 40 запросов даёт 401, реальный Telegram-вход владельца остаётся ручным gate |
...
| Миграции применены                         | Подтверждено                          | `0001`–`0011` применены к обеим D1; 0011 прошла preview-first с Time Travel bookmarks и проверкой числа строк; `PRAGMA foreign_key_check` пуст                            |
...
| Основной App Worker развёрнут              | Подтверждено                          | `https://rolemate-app.carreljeremih.workers.dev`; version `78453a04-6e15-4394-b4c5-ca3adc1e6108`, startup 24 мс, Cron активен                                             |
...
| Автотесты проходят                         | Подтверждено локально                 | 52 unit/integration/migration и 51 E2E прошли; поиск, Premium expiry, rate limit, staff isolation и все admin-вкладки покрыты regression                                  |
