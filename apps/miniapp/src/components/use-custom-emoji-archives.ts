 ...
 */
export function useCustomEmojiArchives(packIds: string[]): Map<string, ArchiveSource> {
  const [urls, setUrls] = useState<Map<string, ArchiveSource>>(new Map());
  ...
  useEffect(() => {
    ...
    const collected = new Map<string, ArchiveSource>();
    ...
    const load = async () => {
      for (const packId of key.split(',')) {
        try {
          ...
          for (const [id, source] of packUrls) collected.set(id, source);
        } catch {
